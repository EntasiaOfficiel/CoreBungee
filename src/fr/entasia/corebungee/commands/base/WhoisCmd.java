	...
	public void execute(CommandSender sender, String[] args) {
		if (sender.hasPermission("staff.whois")) {
			if (args.length == 1) {
				...
				if (target == null)
					sender.sendMessage("§cErreur : " + args[0] + " n'est pas connecté ou n'existe pas!");
				else {
					sender.sendMessage("§e» §6Informations Bungee : §a" + target.getName());
					...
					if (sender.hasPermission("restricted.whoisip")) {
						...
						comp.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, ChatComponent.create("§7Clique pour copier !")));
						sender.sendMessage(comp.create());
					} else sender.sendMessage("§bIP : §cPermissions manquantes");
					...
					sender.sendMessage("§bGrade : §a" + Main.getSuffix(target));
				}
